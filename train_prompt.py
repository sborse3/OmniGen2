...
def parse_args(root_path) -> OmegaConf:
    parser = argparse.ArgumentParser(description="OmniGen2 prompt training script")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
...
def main(args):
    ...
    num_prefix_tokens = args.train.get('num_prefix_tokens', 10)
    text_encoder = PrefixTextEncoder(text_encoder, num_prefix_tokens, weight_dtype)

    if accelerator.is_main_process:
        # Save the original text encoder (without prefix wrapper)
        unwrap_model(text_encoder).text_encoder.save_pretrained(os.path.join(args.output_dir, 'text_encoder'))
    ...
    # Optimizer creation - only optimize prefix tokens
    ...
    for epoch in range(first_epoch, args.train.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                # freqs_cis is already generated at the start of training with model.config.axes_lens
